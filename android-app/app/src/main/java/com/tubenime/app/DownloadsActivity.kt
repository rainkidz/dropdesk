...
class DownloadsActivity : AppCompatActivity() {
    ...
    private fun getDownloadedFiles(): List<DownloadedFile> {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            ...
            val dir = java.io.File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS), "TubeNime")
            if (dir.exists()) {
    ...
    private fun createFileCard(file: DownloadedFile): View {
        ...
        val formatBadge = TextView(this).apply {
            ...
            setBackgroundColor(when {
                ext == "MP4" || ext == "WEBM" || ext == "MKV" -> ContextCompat.getColor(context, R.color.success)
                else -> ContextCompat.getColor(context, R.color.info)
            })
        }
