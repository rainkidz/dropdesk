...
class BrowserActivity : AppCompatActivity() {
    ...
    private fun injectVideoDetection() {
        val js = """
            (function() {
                    ...
                    window._tubenimeObserver = new IntersectionObserver(function(entries) {
                        entries.forEach(function(entry) {
                            if (entry.isIntersecting && entry.intersectionRatio > 0.5) {
                                ...
                                if (video.src) {
                                    window._tubenimeActiveVideo = video.src;
                                }
                            }
                        });
                    }, { threshold: [0.5] });
                ...
                if (window._tubenimeActiveVideo) {
                    videoUrls.unshift(window._tubenimeActiveVideo);
                }
    ...
    private fun handleCustomScheme(url: String) {
        when {
            url.startsWith("tubenime://video_detected?") -> {
