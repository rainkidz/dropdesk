...
class DownloadManager(private val context: Context) {
    ...
    fun download(
        ...
        currentJob = CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                body.byteStream().use { input ->
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                        ...
                        val contentValues = ContentValues().apply {
                            ...
                            put(MediaStore.Downloads.RELATIVE_PATH, Environment.DIRECTORY_DOWNLOADS + "/TubeNime")
                        }
                        ...
                        val dir = File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS), "TubeNime")
                        dir.mkdirs()
                ...
                UsageTracker.record(context, PlatformDetector.detect(url))
    ...
    fun downloadFromUrl(
        ...
        callback: DownloadCallback,
        jobId: String = YtDlpRunner.DEFAULT_JOB
    ) {
        currentJob?.cancel()
        currentJob = CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                when (platform) {
                    ...
                    // Instagram, Threads, Bilibili, and 1000+ other sites
                    Platform.YOUTUBE, Platform.TIKTOK, Platform.FACEBOOK,
                    Platform.INSTAGRAM, Platform.THREADS, Platform.BILIBILI ->
                        downloadWithYtDlp(url, type, formatId, callback, jobId)
    ...
    private suspend fun downloadWithYtDlp(
        ...
        // Cookies (private content) are a Premium feature — never attach them for free users.
        val cookiesFile = cookiesFileFor(url)

        YtDlpRunner.startDownload(context, url, outputPath, format, cookiesFile, jobId)
        ...
        while (!downloadDone) {
            ...
            val progress = YtDlpRunner.getProgress(context, jobId)
            ...
            if (!YtDlpRunner.isDownloading(jobId) && progress.phase == "finalizing") {
                downloadDone = true
                postStatusText(callback, "Download complete!")
                postProgress(callback, 0, 0, 100)
                break
            ...
            if (System.currentTimeMillis() - lastProgressTime > timeoutMs && currentPercent < 100) {
                YtDlpRunner.stopDownload(jobId)
        ...
        val result = YtDlpRunner.getResult(jobId)
        if (result.isFailure) {
        ...
        if (downloadedFile != null && downloadedFile.exists()) {
            // Copy to public Downloads/TubeNime so user can find it in file manager
    ...
    fun downloadMerged(
        ...
        currentJob = CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                YtDlpRunner.startMergeDownload(context, url, outputPath, videoFormat, audioFormat, cookiesFile, jobId)
                ...
                while (!downloadDone) {
                    ...
                    if (!YtDlpRunner.isDownloading(jobId) && (progress.phase == "finalizing" || progress.phase == "merging")) {
                        downloadDone = true
                        postStatusText(callback, "Merge complete!")
                        postProgress(callback, 0, 0, 100)
                        break
    ...
    fun downloadPlaylist(
        ...
        currentJob = CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                YtDlpRunner.startPlaylistDownload(context, url, outputPath, format, maxVideos, cookiesFile = null, jobId = jobId)
    ...
    /**
     * Cookies unlock private/restricted content, which is a Premium feature.
     * Free users never get cookies attached, so private downloads fail cleanly
     * (and the UI translates that into an upgrade prompt).
     */
    private fun cookiesFileFor(url: String): String? {
    ...
    /**
     * Copy file from internal storage to public Downloads/TubeNime/ folder.
